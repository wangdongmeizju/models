...
DATA_HOME = os.path.expanduser(args.datadir)
...
binary = fluid.compiler.CompiledProgram(fluid.default_main_program()).with_data_parallel( loss_name=avg_cost.name)
...
for pass_id in range(50):                                                                                                         
        ...
        for batch_id,data in enumerate(train_reader()):                                                                          
                ...
                train_cost,train_acc=exe.run(program=binary,feed=feeder.feed(data),fetch_list=[avg_cost,accuracy])                                                                                                                          
                #每100个batch打印一次信息                                                                                        
                if batch_id%100==0:                                                                                              
