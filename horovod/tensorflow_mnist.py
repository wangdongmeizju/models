...
def conv_model(feature, target, mode):
    ...
    accuracy = tf.reduce_mean(tf.cast(tf.equal(tf.argmax(logits,1), tf.argmax(target,1)), tf.float32))
...
def main(_):
    ...
    #predict, loss, accuracy = conv_model(image, label, tf.contrib.learn.ModeKeys.TRAIN)
    predict, loss, accuracy = conv_model(image, label, tf.contrib.learn.ModeKeys.TRAIN)
    ...
    hooks = [
        ...
        tf.train.StopAtStepHook(last_step=FLAGS.step // hvd.size()),

        tf.train.LoggingTensorHook(tensors={'step': global_step, 'loss': loss, 'acc': accuracy},
                                   every_n_iter=10),
    ]
    ...
    # corrupting them.
    checkpoint_dir = FLAGS.model_save_path if hvd.rank() == 0 else None
