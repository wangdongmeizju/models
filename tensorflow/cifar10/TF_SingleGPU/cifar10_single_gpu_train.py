...
#tf.app.flags.DEFINE_string('model_save_path', '/inpsur/models/tensorflow/cifar10/TF_SingleGPU/log_single/',
tf.app.flags.DEFINE_string('model_save_path', '/wangdongmei/models/logs/log_tf_cifar_single/',
...
def train():
  ...
  with tf.Graph().as_default(), tf.device('/cpu:0'):
   ...
   # images, labels = cifar10_input._generate_image_and_label_batch(shuffle=True, batch_size=FLAGS.batch_size)
   # logits = cifar10.inference(images)
    ...
    for step in xrange(FLAGS.max_step):
     ...
     # _, loss_value,accuracy_value = sess.run([train_op, loss, accuracy])
      ...
      if step % 10 == 0:
       ...
       # format_str = ('%s: step %d, loss = %.2f (%.1f examples/sec; %.3f sec/batch; %.2f Train accuracy )')
